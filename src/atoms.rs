...
pub fn replacement_for(typ: &str) -> (Option<&'static str>, Option<&'static str>) {
    let replacement = STDLIB_TYPE_REPLACEMENTS.get(typ).copied();
    let lib = DART_LIBRARIES_FOR_BROWSER_TYPES.get(typ).copied();
    (replacement, lib)
}

type AtomMap = AHashMap<&'static str, &'static str>;
